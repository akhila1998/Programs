...
void print_chromosomes(vector<elementOfChromosome> tempchromo) {
	...
	ofstream outputfile;
	outputfile.open("TimeTableEvenSem.txt");
...
beginagain:
	...
	while (ans == "2") {
		...
		cout << endl << "Third year done" << endl << endl;
	...
	secondyear:
		...
		cout << endl << "Second Year is done" << endl << endl;
	...
	fourthyear:
		...
		while (tempnum != 1) {
			...
			for (int i = 0; i<2000; i++) {
				...
				tempnum = 1;//fillin4thyear(dupchromosome,templistofcourses,listofbatches,teacherunav);
				if (tempnum != 1) {
					dupchromosome.erase(dupchromosome.begin(), dupchromosome.end());
					templistofcourses.erase(templistofcourses.begin(), templistofcourses.end());
					oldtempvec.erase(oldtempvec.begin(), oldtempvec.end());
